...
`my_fn` 返回一个 [Future](https://doc.rust-lang.org/std/future/trait.Future.html)

当等待 Future 时，我们的异步运行时 (tokio) 就会介入: 它开始[轮询](https://doc.rust-lang.org/std/future/trait.Future.html#tymethod.poll) Future。
...
> 返回收集器认为是当前跨度的跨度句柄。如果收集器指示它不跟踪当前跨度，或者**调用此函数的线程**当前不在跨度内，则返回的跨度将被禁用。
